...
# Build address; allow a fully-formed override via NAKAMA_DATABASE_ADDRESS
DB_ADDRESS="${NAKAMA_DATABASE_ADDRESS:-${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}?sslmode=${DB_SSL}}"

echo "==> Starting Nakama migration: ${DB_HOST}:${DB_PORT}/${DB_NAME} (ssl=${DB_SSL})"
/nakama/nakama migrate up --database.address "${DB_ADDRESS}" && echo "==> Migration OK"

echo "==> Starting Nakama on 0.0.0.0:${PUBLIC_PORT}"
exec /nakama/nakama \
